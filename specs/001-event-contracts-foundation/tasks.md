...
- [x] T050 [US3] Verify Scenario 7: `./mvnw clean verify` from a clean checkout builds and tests all modules in dependency order (SC-004)
- [x] T051 [US3] Verify SC-008: add a throwaway empty module, confirm the only edits needed are one `<module>` line in the root `pom.xml` plus the new module's own `pom.xml`, then remove it
